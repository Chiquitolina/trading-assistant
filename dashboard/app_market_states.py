...
df_trades = load_backtest_trades() if AGGRESSIVE_MODE else pd.DataFrame()

# =========================
# EMA OPTIONS
# =========================
show_ema20 = st.sidebar.checkbox("EMA 20", value=False)
show_ema34 = st.sidebar.checkbox("EMA 34", value=False)
...
def build_trade_markers(df_trades):
    ...
    for _, row in df_trades.iterrows():
        ...
        markers.append({
            ...
            "color": "#00e676" if row["side"] == "LONG" else "#ff1744",
            "text": f"{row['side']} {pnl:.2f}"
        })
...
if show_ema34:
    ema34_data = [
        {
            "time": int(row["time"]),
            "value": float(row["ema34"])
        }
        for _, row in df_chart.iterrows()
        if pd.notna(row.get("ema34"))
    ]
...
], key=f"market_states_chart_{chart_tf}_{show_trend}_{show_shift}_{show_regime}_{show_direction}_{show_direction_stats}_{show_momentum}_{show_ema20}_{show_ema34}_{show_ema50}_{'_'.join(selected_momentums)}")
