...
class LegProtectionManager:
    ...
    def place(self, leg, plan):
        ...
        try:
            ...
            self._cancel(leg.symbol, self._order_id(sl_order))
            raise
        protection = LegProtection(
            ...
            self._order_id(tp_order), self._order_id(sl_order),
        )
        leg.tp, leg.sl = protection.tp_price, protection.sl_price
        leg.tp_order_id, leg.sl_order_id = protection.tp_order_id, protection.sl_order_id
        self.by_leg[leg.leg_id] = protection
        self.order_to_leg[protection.tp_order_id] = leg.leg_id
        self.order_to_leg[protection.sl_order_id] = leg.leg_id
        self.verify_leg(leg)
        return protection
    ...
    def verify_leg(self, leg):
        ...
        orders = {int(x["orderId"]): x for x in getter(leg.symbol)}
    ...
    def verify_position(self, position):
        getter = getattr(self.exchange, "get_all_protective_orders", self.exchange.get_open_orders)
        orders = {int(x["orderId"]): x for x in getter(position.symbol)}
        tp_total = sl_total = 0.0
        for leg in position.entry_legs:
            if leg.remaining_quantity <= 1e-9: continue
            self.verify_leg(leg)
            for order_id, family in ((leg.tp_order_id, "tp"), (leg.sl_order_id, "sl")):
    ...
    def process_fill(self, position, leg, fill):
        ...
        sibling = leg.sl_order_id if filled_order == leg.tp_order_id else leg.tp_order_id
        if leg.remaining_quantity <= 1e-9:
            ...
            if sibling == leg.sl_order_id:
                ...
                leg.sl_order_id = protection.sl_order_id = self._order_id(order)
                self.order_to_leg[leg.sl_order_id] = leg.leg_id
            else:
