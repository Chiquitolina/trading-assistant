...
def detect_breakout_from_watch(
    df: pd.DataFrame,
    compression_high: float,
    volume_lookback: int = 20,
    min_volume_expansion: float = 1.5,
):
    if df is None or len(df) < volume_lookback + 2:
        return {
            "breakout": False,
    ...
    if close_breaks_high:
        passed_reasons.append("close_breaks_watch_high")
    else:
        failed_reasons.append(
            f"close_not_above_watch_high "
            f"close={float(last['close']):.8f} "
            f"watch_high={compression_high:.8f}"
        )

    if high_breaks_high:
        passed_reasons.append("high_breaks_watch_high")
    else:
        failed_reasons.append(
            f"high_not_above_watch_high "
    ...
    return {
        ...
        "reason": "watch_compression_breakout" if breakout else "breakout_conditions_not_met",
